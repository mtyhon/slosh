...
class NPZ_Dataset_OnTheFly(data.Dataset):
    ...
    def __init__(self, filenames, kic, mag, numax, labels, dim, add_noise,random_draws=False, scale=False, validate=False):
    ...
    def __data_generation(self, batch_filenames, batch_mag, batch_numax):
        ...
        im = ps_to_array(freq, power, minfreq=3., scale=self.scale, fix=True) ###############################################     CHANGE MIN FREQ????????????????????????    ##################################
...
def train_model_bell_array():
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if model_checkpoint:
            ...
            if is_best:
                filename = '27d_FIXEDSCALE_SLOSCFilter_Drop=0.1_NoAugment-Loss:%.2f-Acc:%.2f' % (
                val_loss, val_acc)
