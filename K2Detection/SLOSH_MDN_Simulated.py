...
root_folder = '/data/marc/KEPSEISMIC/LC_Data/PSD/npz_PSD_numax/1/'
...
def train_model():
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if model_checkpoint:
            ...
            if is_best:
                filename = '27d_REALLYFIXEDSCALE_SLOSCFilter_Drop=0.1-MAPE:%.2f-MAE:%.2f' % (
                val_mape, val_mae)
...
def test_model():
    ...
    val_gen = NPZ_Dataset_OnTheFly(filenames = val_filenames,kic=val_kic, dim=(128,128), scale=False, validate=True)
    val_dataloader = utils.DataLoader(val_gen, shuffle=True, batch_size=32, num_workers=5)
    ...
    #saved_model = torch.load('/home/z3384751/K2Detection/ClassifyModels/2D_classifier/Pytorch_MDN_SLOSH/27d_REALLYFIXEDSCALE_SLOSCFilter_Drop=0.1-MAPE:5.52-MAE:3.49') ## fixed scale?
    ...
    saved_model = torch.load('/home/z3384751/K2Detection/ClassifyModels/2D_classifier/Pytorch_MDN_SLOSH/SLOSCFilter_Drop=0.1_NoAugment-MAPE:3.45-MAE:2.22') ## fixed scale?
    ...
    np.savez_compressed('OLD_SLOSC_FIXED_REGRESSOR_NETWORK1_VALDATA', id=val_id_vec, label=val_label_vec, pred=val_pred_vec, var=val_var_vec, mag=val_mag_vec, hb=val_hb_vec)
...
def test_model_jie():
    ...
    np.savez_compressed('SLOSC_REALLYFIXED_JIEPRED_SCALED_REGRESSOR_VALDATA', id=val_id_vec, label=val_label_vec, pred=val_pred_vec, var=val_var_vec)
